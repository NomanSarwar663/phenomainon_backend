...
async function insertTask(req, res) {
    try {
        const response = await taskService.insertTask(req.body, req.user._id);
        if (response) {
            return res
...
async function getTasks(req, res) {
    try {
        const response = await taskService.getTasks(req.user._id);
        if (response) {
            return res
                .status(response.statusCode)
                .json(response);
...
async function deleteTask(req, res) {
    try {
        const { id } = req.params
        const response = await taskService.deleteTask(id, req.user._id);
        if (response) {
            return res
                .status(response.statusCode)
                .json(response);
